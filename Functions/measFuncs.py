...
""" Things need to check it gets right

NEED TO FIGURE OUT HOW TO GET MEDIAN TSYS FROM MS
...
def measSetInfo(msname,wantSPW,myfield=''):
    ...
    return numAnts, outDiamM[0], dataBWs[wantSPW], (obsTime*60.0)/float(len(target_fields)), np.asarray(target_fields), chan_res[wantSPW], mySourceIdx
...
def getSourcePos(targImage,specPath):
        ...
        imstat_dict=imstat(imagename=targImage, axes=[0,1], region=boxStr, box="", chans="", stokes="", listit=True, verbose=True, mask="", stretch=False, logfile="", append=True, algorithm='classic',fence=-1, center="mean", lside=True, zscore=-1, maxiter=-1, clmethod="auto")
